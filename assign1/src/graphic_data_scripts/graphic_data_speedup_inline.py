...
function_types = ["Inline Mult", "Parallelized Inline Mult", "Inner Most Loop Parallelization Inline"]
...
serial_times_dict = {}
for _, row in filtered_data[filtered_data['functionType'] == 'Inline Mult'].iterrows():
...
for _, row in filtered_data.iterrows():
    ...
    # For serial execution (Inline Mult), speedup is 1.0 and efficiency is 1.0
    if row['functionType'] == 'Inline Mult':
        speedup = 1.0
        efficiency = 1.0
    else:
...
def create_label(func_type):
    labels = {
        "Inline Mult": "Inline Serial Execution",
        "Parallelized Inline Mult": "Outer Loop Parallelization (Inline)",
...
colors = {
    "Inline Mult": "#1f77b4",
    "Parallelized Inline Mult": "#ff7f0e",
...
function_types = [ft for ft in results_df['functionType'].unique() if ft != "Inline Mult"]
...
plt.title('SpeedUp Comparison by Implementation and Matrix Size (Inline Functions)', fontweight='bold', pad=20)
...
plt.title('Parallel Efficiency Comparison by Implementation and Matrix Size (Inline Functions)', fontweight='bold', pad=20)
plt.xticks(r + barWidth/2 - barWidth/(2*len(function_types)), [str(int(size)) for size in matrix_sizes])
plt.legend(loc='upper right', frameon=True, fancybox=True, shadow=True)
plt.grid(axis='y', linestyle='--', alpha=0.7)

# Add an overall title
plt.suptitle("Performance Comparison of Inline Function Parallelization Strategies", fontsize=18, fontweight='bold', y=0.98)

plt.tight_layout()
plt.subplots_adjust(hspace=0.3, top=0.92)
plt.savefig('inline_parallel_performance_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
print("\nAverage Performance Metrics Summary for Inline Functions:")
print(summary_df)
...
results_df.to_csv('inline_performance_metrics_results.csv', index=False, float_format='%.3f')